...
createDB dbPath dbName = do
	...
	dbPathVar <- return $ ValD (VarP $ mkName $ (dbName) ++ "Path") (NormalB (LitE (StringL  dbPath))) []
	...
	dbCheck <- return $ ValD (VarP $ mkName $ "check" ++ (toTitleCase dbName) ++ "Consistency") (NormalB (AppE (AppE (VarE $ mkName "checkDbHash") ((LitE (IntegerL $ toInteger (hash dbInfo))))) (LitE (StringL  dbPath)))) []
	dbCheck' <- return $ ValD (VarP $ mkName $ "check" ++ (toTitleCase dbName) ++ "Consistency'") (NormalB (AppE (AppE (VarE $ mkName "checkDbHash'") ((LitE (IntegerL $ toInteger (hash dbInfo))))) (LitE (StringL  dbPath)))) []
	-- declFromDB' <- defFromDB' dbName dbPath
